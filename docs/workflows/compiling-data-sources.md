...
You will use **`1-gather_all_flu_datasets_ipynb.py`** to turn surveillance records and simulated influenza trajectories into **`Flusight/flu-datasets/all_datasets.parquet`**, a common table that the training-data notebook can mix.
...
```bash
jupytext --to notebook 1-gather_all_flu_datasets_ipynb.py --output 1-gather_all_flu_datasets.ipynb
```
