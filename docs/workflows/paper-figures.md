...
You will use **`paper_figures.final_figures` and the reproduction archive** to create **the eight data figures in the paper and supplement**, plus correlation and operational leaderboard summaries.
...
## Follow each figure back to its inputs
...
## Use the outputs in the manuscript

Archive-mode outputs are separate from the images included by `influpaint-paper/main.tex` and `supplementary_information.tex`. Review the regenerated plots before copying them to the manuscript's `figures/` paths. The static `figures/methods.png` diagram is already part of the manuscript and is not generated by this command.

[Previous: 7. Reconstruct missing observations](mask-experiments.md) · [Back to contents](start-here.md)
