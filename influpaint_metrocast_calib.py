...
# %%
def parse_args():
    parser = argparse.ArgumentParser(description="MetroCast finetune + inpainting calibration sweep")
    parser.add_argument("--scenario_id", type=int, default=868)
    parser.add_argument("--forecast_date", type=str, default="2026-01-17")
    parser.add_argument("--config_name", type=str, default="celebahq_noTTJ5")
    parser.add_argument("--batch_size", type=int, default=256)
    parser.add_argument("--image_size", type=int, default=128)
    parser.add_argument("--channels", type=int, default=1)
    parser.add_argument("--finetune_mode", type=str, default="adapters_time_ups2")
    parser.add_argument("--finetune_epochs", type=int, default=50)
    parser.add_argument("--finetune_lr", type=float, default=1e-4)
    parser.add_argument("--metrocast_nc_path", type=str, default="training_datasets/MetrocastTS_100M_2026-01-16.nc")
    parser.add_argument("--do_uncond_preview", type=lambda x: str(x).lower() == "true", default=True)
    parser.add_argument("--uncond_batch_size", type=int, default=8)
    parser.add_argument("--experiment_name", type=str, default="paper-2025-07-22_training")
    parser.add_argument("--run_id", type=str, default=None)
    parser.add_argument("--model_path", type=str, default=None)
    parser.add_argument("--calib_output_root", type=str, default="output/metrocast_calib")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--train_finetune", dest="train_finetune", action="store_true")
    group.add_argument("--no_train_finetune", dest="train_finetune", action="store_false")
    parser.set_defaults(train_finetune=True)
...
# === USER CONFIGURATION ===
scenario_id = args.scenario_id  # Choose your training scenario
forecast_date = args.forecast_date  # YYYY-MM-DD format
config_name = args.config_name  # CoPaint config name
batch_size = args.batch_size
image_size = args.image_size
channels = args.channels
train_finetune = args.train_finetune
finetune_mode = args.finetune_mode
finetune_epochs = args.finetune_epochs
finetune_lr = args.finetune_lr
metrocast_nc_path = Path(args.metrocast_nc_path)
do_uncond_preview = args.do_uncond_preview
uncond_batch_size = args.uncond_batch_size
experiment_name = args.experiment_name
run_id = args.run_id
model_path = args.model_path

calib_output_root = Path(args.calib_output_root)
...
run_tag = f"{scenario_spec.scenario_string}::finetune_{finetune_epochs}_{finetune_lr}_{finetune_mode}"
calib_run_dir = calib_output_root / run_tag
finetune_output_dir = calib_run_dir / "finetune"
plots_dir = calib_run_dir / "plots"
...
if train_finetune:
    ...
    finetune_ckpt = finetune_output_dir / f"{run_tag}.pth"
...
if do_uncond_preview:
    ...
    plt.tight_layout()
    uncond_plot_path = plots_dir / f"{run_tag}__uncond_preview.png"
    plt.savefig(uncond_plot_path, dpi=300, bbox_inches="tight")
    plt.show()
...
plt.tight_layout()
traj_plot_path = plots_dir / f"{run_tag}__trajectories.png"
plt.savefig(traj_plot_path, dpi=300, bbox_inches="tight")

# %%
# Create output directory
output_dir = calib_run_dir / "forecasts" / str(submission_date)
output_dir.mkdir(parents=True, exist_ok=True)

# Export using ground truth helper
team_abbrv = "UNC_IDD-InfluPaint"
export_prefix = run_tag
