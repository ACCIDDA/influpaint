...
write_parquet(nssp, "RSV/data/NSSP.parquet")
```
...
rsvnet |>
  ...
  as_tibble() |>
  select(skim_variable, complete_rate)
write_parquet(rsvnet, "RSV/data/RSV_NET.parquet")
```
...
write_parquet(nhsn, "RSV/data/NHSN.parquet")
```
...
write_parquet(rsv_smh, "RSV/data/RSV_SMH.parquet")
```
...
nssp <- read_parquet("RSV/data/NSSP.parquet")
nhsn <- read_parquet("RSV/data/NHSN.parquet")
...
write_parquet(nssp_counts, "RSV/data/NSSP_COUNTS.parquet")
...
```{r, all_data}
set.seed(123)
rsv_smh20 <- read_parquet("RSV/data/RSV_SMH.parquet") |>
  group_by(datasetH2) |>
...
rsv_all <- bind_rows(
  read_parquet("RSV/data/NSSP_COUNTS.parquet"),
  read_parquet("RSV/data/RSV_NET.parquet"),
  read_parquet("RSV/data/NHSN.parquet"),
  rsv_smh20
) |>
  #drop 2025
  filter(fluseason < 2025)

write_parquet(rsv_all, "RSV/data/RSV_ALL.parquet")
```
...
write_parquet(rsv_filled, "RSV/data/RSV_FILLED.parquet")
```
...
```{r, sanity}
set.seed(123)
rsv_smh <- read_parquet("RSV/data/RSV_SMH.parquet")
...
```{python}
all_datasets_df = pd.read_parquet("RSV/data/RSV_FILLED.parquet")
...
```{r}
n_frames <- arrow::read_parquet("RSV/data/RSV_FILLED.parquet") |>
...
## Inspect array
```{python}
arr = xr.open_dataarray("training_datasets/RSV_100S_2026-04-24.nc")
