...
def pad_dataframe(df, season_setup):
    ...
    expanded_df = pd.DataFrame(
        [(d, l) for d in date_range for l in locations],
        columns=["week_enddate", "location_code"],
    )

    # Calculate the season columns
    expanded_df["fluseason"] = expanded_df.week_enddate.apply(
        season_setup.get_fluseason_year
    )
    expanded_df["fluseason_fraction"] = expanded_df.week_enddate.apply(
        season_setup.get_fluseason_fraction
    )
    expanded_df["season_week"] = expanded_df.week_enddate.apply(
        season_setup.get_season_week
    )
...
class GroundTruth:
    def __init__(
        ...
        self.season_setup = SeasonAxis.for_flusight(
            remove_territories=True, remove_us=True
        )

        flusight = pd.read_parquet(
            "https://raw.githubusercontent.com/CDCgov/rsv-forecast-hub/main/target-data/oracle-output.parquet",
            engine="auto",
        )

        flusight = self.season_setup.add_season_columns(
            flusight, do_fluseason_year=True
        )
        ...
        if from_final_data:
            ...
            flusight = read_datasources.get_from_epidata(
                dataset=f"flusight{self.season_first_year}",
        ...
        self.gt_df_final = gt_df_final[
            gt_df_final["location_code"].isin(self.season_setup.locations)
        ]

        # generates past data
        past_data_1 = read_datasources.get_from_epidata(
            dataset=f"flusight2024", season_setup=self.season_setup, write=False
        )
        past_data_2 = read_datasources.get_from_epidata(
            dataset=f"flusight2024", season_setup=self.season_setup, write=False
        )

        # Add season columns to past data
        past_data_1 = self.season_setup.add_season_columns(
            past_data_1, do_fluseason_year=True
        )
        past_data_2 = self.season_setup.add_season_columns(
            past_data_2, do_fluseason_year=True
        )
        ...
        last_non_nan_datadate = (
            self.gt_df.week_enddate[self.gt_df.value.notna()].max().to_pydatetime()
        )
        ...
        if self.mask_date > last_non_nan_datadate + datetime.timedelta(days=7):
            ...
            print(
                f" WARNING: mask_date is after last non-NaN data date, setting mask_date to {self.mask_date}"
            )
        ...
        if payload is not None:
            ...
            payload = self.season_setup.add_season_columns(
                payload, do_fluseason_year=True
            )
            this_payload = payload[
                payload["fluseason"] == int(payload_season_first_year)
            ]
            self.gt_df = pd.concat([self.gt_df, this_payload], ignore_index=True)
            self.gt_df_final = pd.concat(
                [self.gt_df_final, this_payload], ignore_index=True
            )
            ...
            new_locations = new_locations.merge(
                self.season_setup.locations_df, on="location_code", how="left"
            )
        ...
        self.previous_data = pd.concat(
            self.previous_data, ignore_index=True
        ).drop_duplicates()
        self.previous_data = self.season_setup.add_season_columns(
            self.previous_data, do_fluseason_year=True
        )
        ...
        print(
            f"Masking, >> {self.inpaintfrom_idx} weeks already in data, inpainting the next ones"
        )
    ...
    def git_checkout_data_rev(self, target_date=None):
        ...
        if target_date is not None:
            ...
            if closest_commit:
                ...
                print(
                    "ERROR: No commit found for the specified date on repo {repo_path}."
                )
        else:
    ...
    def plot(self):
        season_start_date = datetime.date(
            ...
            self.season_setup.season_start_month,
            self.season_setup.season_start_day,
        )
        fig, axes = plt.subplots(8, 8, sharex=True, figsize=(14, 16))
        gt_piv = self.gt_df.pivot(
            index="week_enddate", columns="location_code", values="value"
        )
        gt_piv_final = self.gt_df_final.pivot(
            index="week_enddate", columns="location_code", values="value"
        )
        ax = axes.flat[0]
        ax.plot(
            gt_piv[self.season_setup.locations].sum(axis=1),
            color="black",
            linewidth=2,
            label="datadate",
        )
        ax.plot(
            gt_piv_final[self.season_setup.locations].sum(axis=1),
            lw=1,
            color="r",
            ls="-.",
            label="final",
        ...
        for idx, pl in enumerate(gt_piv.columns):
            ...
            ax.set_ylim(0)
            ax.set_xlim(
                season_start_date, season_start_date + datetime.timedelta(days=365)
            )
    ...
    def plot_mask(self):
        ...
        fig, axes = plt.subplots(
            1, 4, figsize=(8, 8), dpi=200, sharex=True, sharey=True
        )
    ...
    def export_forecasts(
        ...
        target_dates = pd.date_range(
            forecast_date, forecast_date + datetime.timedelta(days=4 * 7), freq="W-SAT"
        )

        target_dict = dict(
            zip(target_dates, [f"{n} wk ahead inc flu hosp" for n in range(1, 5)])
        )
        ...
        for qt in myutils.flusight_quantiles:
            a = pd.DataFrame(
                np.quantile(
                    fluforecasts_ti[:, :, :, : len(self.season_setup.locations)],
                    qt,
                    axis=0,
                )[0],
                ...
                index=pd.date_range(
                    season_start_date,
                    season_start_date + datetime.timedelta(days=64 * 7),
                    freq="W-SAT",
                ),
            ).loc[target_dates]
            # a["US"] = a.sum(axis=1)
        ...
        for tg in target_dates:
            ...
            for dfd in df_list:  # very important to not call this df: it overwrites in namesapce the exported df
                ...
                if not (new_vals - old_vals >= 0).all():
                    ...
                    print(
                        f" !!!! Quantile validation failed: {num_negative} negative values on {tg}"
                    )
                else:
                    pass
    ...
    def plot_forecasts(
        ...
        plot_specs = {
            ...
            "50-95": {"quantiles_idx": [1, 6], "color": "darkblue"},
        }
        ...
        for plot_title, plot_spec in plot_specs.items():
            ...
            fig, axes = plt.subplots(
                nplace_toplot + 1, 2, figsize=(10, nplace_toplot * 3.5), dpi=200
            )
            for iax in range(2):
                ax = axes[0][iax]
                ...
                if iax == 0:
                    x_lims_idx = (0, 51)
                    x_lims = (
                        pd.to_datetime(self.gt_xarr["date"][x_lims_idx[0]].values),
                ...
                for iqt in plot_spec["quantiles_idx"]:
                    ...
                    ylo = np.quantile(
                        forecasts_national,
                        myutils.flusight_quantile_pairs[iqt, 0],
                        axis=0,
                    )[0]
                    yup = np.quantile(
                        forecasts_national,
                        myutils.flusight_quantile_pairs[iqt, 1],
                        axis=0,
                    )[0]
                    ax.fill_between(
                    ...
                    if iqt == plot_spec["quantiles_idx"][0]:
                        ...
                        max_y_value = max(
                            max_y_value,
                            self.gt_xarr.data[0, : self.inpaintfrom_idx]
                            .sum(axis=1)[x_lims_idx[0] : x_lims_idx[1]]
                            .max(),
                        )
                        max_y_value = max_y_value + max_y_value * 0.05  # 10% more

                # median
                ax.plot(
                    self.gt_xarr["date"][plotrange],
                    np.quantile(
                        forecasts_national, myutils.flusight_quantiles[12], axis=0
                    )[0][plotrange],
                    color=plot_spec["color"],
                    marker=".",
                ...
                # ground truth
                ax.plot(
                    self.gt_xarr["date"][: self.inpaintfrom_idx],
                ...
                if iax == 0:
                    ax.axvline(
                        self.gt_xarr["date"][idx_horizon].values, c="k", lw=1, ls="-."
                    )
                ...
                for iqt in plot_spec["quantiles_idx"]:
                    yup = np.quantile(
                        fluforecasts_ti, myutils.flusight_quantile_pairs[iqt, 0], axis=0
                    )[0]
                    ylo = np.quantile(
                        fluforecasts_ti, myutils.flusight_quantile_pairs[iqt, 1], axis=0
                    )[0]
                    ...
                    if iqt == plot_spec["quantiles_idx"][0]:
                        for ipl in range(nplace_toplot):
                            if plot_past_median:
                                max_y_value[ipl] = max(
                                    ylo[x_lims_idx[0] : x_lims_idx[1], ipl]
                                )
                            else:
                                max_y_value[ipl] = max(
                                    ylo[self.inpaintfrom_idx : x_lims_idx[1], ipl]
                                )
                            # max_y_value[ipl] =  max(ylo[x_lims[:x_lims[1], ipl])
                            max_y_value[ipl] = max(
                                max_y_value[ipl],
                                self.gt_xarr.data[0, : self.inpaintfrom_idx, ipl][
                                    x_lims_idx[0] : x_lims_idx[1]
                                ].max(),
                            )
                            max_y_value[ipl] = (
                                max_y_value[ipl] + max_y_value[ipl] * 0.05
                            )  # 10% more for the y_max value
                ...
                for ipl in range(nplace_toplot):
                    ...
                    ax.plot(
                        self.gt_xarr["date"][plotrange],
                        np.quantile(
                            fluforecasts_ti, myutils.flusight_quantiles[12], axis=0
                        )[0, :, ipl][plotrange],
                        color=plot_spec["color"],
                        marker=".",
                        lw=0.5,
                    ...
                    this_hist_data = self.previous_data[
                        self.previous_data["location_code"]
                        == self.season_setup.locations[ipl]
                    ]
                    for hist_season in this_hist_data["fluseason"].unique():
                        if int(hist_season) != int(self.season_first_year):
                            hist_data = this_hist_data[
                                this_hist_data["fluseason"] == hist_season
                            ]
                            hist_data = hist_data.pivot(
                                index="season_week",
                            ...
                            # TODO MATCH HERE !!!!
                            ax.plot(
                                self.gt_xarr["date"][0 : len(thisthing)],
                                thisthing,
                                color=color_past,
                                ls="dashed",
                                lw=0.5,
                    ...
                    ax.axvline(
                        self.gt_xarr["date"][idx_now].values, c="k", lw=1, ls="-."
                    )
                    if iax == 0:
                        ax.axvline(
                            self.gt_xarr["date"][idx_horizon].values,
                            c="k",
                            lw=1,
                            ls="-.",
                        )
                    ...
                    if iax == 0:
                        ax.set_ylabel("New Hosp. Admissions")
            ...
            fig.tight_layout()
            plt.savefig(
                f"{directory}/{prefix}-{forecast_date_str}-plot{plot_title}.pdf"
            )

    def export_forecasts_2023(
        self,
        fluforecasts_ti,
        forecasts_national,
        directory=".",
        ...
        target_dates = pd.date_range(
            forecast_date, forecast_date + datetime.timedelta(days=3 * 7), freq="W-SAT"
        )

        target_dict = dict(zip(target_dates, [f"{n}" for n in range(0, 4)]))
