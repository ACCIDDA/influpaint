...
def _validate_required_columns(
    df: pd.DataFrame, required_columns: list, context: str
) -> None:
...
def build_frames(
    ...
    fill_missing_locations: str = "error",
    scaling_distribution: np.ndarray = None,
) -> list:
    """
    ...
    if needs_scaling and scaling_distribution is None:
        raise ValueError(
            "scaling_distribution must be provided when any dataset has to_scale=True"
        )
    ...
    for h1_name, summary in frame_summary.items():
        print(
            f"  {h1_name}: {summary['total_frames']} frames (multiplier={summary['multiplier']})"
        )
...
def _calculate_proportional_multipliers(
    all_datasets_df: pd.DataFrame, config: dict
) -> dict:
    ...
    for h1_name, h1_config in config.items():
        if "proportion" not in h1_config or "total" not in h1_config:
            raise ValueError(
                f"Config for '{h1_name}' must have both 'proportion' and 'total'"
            )
...
def _build_h1_frames(
    ...
    all_datasets_df: pd.DataFrame = None,
    global_lookup: dict = None,
...
def _pad_frame_complete(
    ...
    if missing_locations:
        frame = _handle_missing_locations(
            frame,
            missing_locations,
            actual_locations,
            fill_missing_locations,
    ...
    return final_frame.sort_values(["location_code", "season_week"]).reset_index(
        drop=True
    )


def _handle_missing_locations(
    frame: pd.DataFrame,
    ...
    if strategy == "error":
        ...
        origin = (
            frame["origin"].iloc[0]
            if "origin" in frame.columns and not frame.empty
            else "unknown"
        )
        h1 = (
            frame["datasetH1"].iloc[0]
            if "datasetH1" in frame.columns and not frame.empty
            else "unknown"
        )
        h2 = (
            frame["datasetH2"].iloc[0]
            if "datasetH2" in frame.columns and not frame.empty
            else "unknown"
        )
        season = (
            frame["fluseason"].iloc[0]
            if "fluseason" in frame.columns and not frame.empty
            else "unknown"
        )
        sample = (
            frame["sample"].iloc[0]
            if "sample" in frame.columns and not frame.empty
            else "unknown"
        )

        raise ValueError(
            f"Missing required locations: {missing_locations}\n"
        ...
        return _fill_missing_with_random(
            frame, missing_locations, actual_locations, all_datasets_df, global_lookup
        )


def _fill_missing_with_zeros(
    frame: pd.DataFrame, missing_locations: set
) -> pd.DataFrame:
...
def _fill_missing_with_random(
    ...
    **Each multiplied frame is processed independently:**
    ...
    for location in missing_locations:
        fill_data, fill_source = _get_fill_data_from_global_lookup(
            frame, location, global_lookup, actual_locations
        )
...
def _build_global_lookup_table(all_datasets_df: pd.DataFrame) -> dict:
    ...
    for location, location_data in location_groups:
        ...
        for h1_name, h1_data in h1_groups:
            lookup[location][h1_name] = {"by_year": {}, "by_sample": {}}
            ...
            for year, year_data in year_groups:
                ...
                if not unique_combos.empty:
                    ...
                    lookup[location][h1_name]["by_year"][year] = {
                        "data": specific_data[
                            ["season_week", "value", "week_enddate"]
                        ].copy(),
...
def _get_fill_data_from_global_lookup(
    frame: pd.DataFrame, location: str, global_lookup: dict, actual_locations: set
) -> tuple:
    ...
    if global_lookup is None or location not in global_lookup:
        raise ValueError(
            f"Location {location} not found anywhere in the dataset. Cannot fill missing location."
        )

    # Get frame metadata
    current_h1 = frame["datasetH1"].iloc[0] if "datasetH1" in frame.columns else None
    current_season = (
        frame["fluseason"].iloc[0] if "fluseason" in frame.columns else None
    )
    ...
    search_strategies = [
        ...
        lambda: _find_same_h1_different_year(
            location_lookup, current_h1, current_season
        ),
        # 2. Same H1, different sample
        lambda: _find_same_h1_different_sample(
            location_lookup, current_h1, current_season, current_sample
        ),
        # 3. Different H1
    ...
    raise ValueError(
        f"Location {location} found in dataset but no valid fill data available."
    )


def _find_same_h1_different_year(
    location_lookup: dict, current_h1: str, current_season: int
) -> tuple:
    ...
    available_years = [
        year for year in h1_lookup["by_year"].keys() if year != current_season
    ]
...
def _find_same_h1_different_sample(
    location_lookup: dict, current_h1: str, current_season: int, current_sample: str
) -> tuple:
    ...
    available_samples = [
        key
        for key, info in h1_lookup["by_sample"].items()
...
def _preserve_columns(
    target_df: pd.DataFrame, source_df: pd.DataFrame, location_data: pd.DataFrame
) -> None:
...
def _pad_single_location(frame: pd.DataFrame, location: str) -> pd.DataFrame:
    ...
    if frame.empty:
        ...
        missing_data = [
            {"season_week": week, "location_code": location, "value": 0}
            for week in range(1, 54)
        ]
    ...
    for week in missing_weeks:
        ...
        if week < min_week or week > max_week:
            ...
            new_value = (
                previous_week["value"].values[0] if not previous_week.empty else 0
            )
...
def _apply_frame_scaling(
    frame: pd.DataFrame, scaling_distribution: np.ndarray
) -> pd.DataFrame:
    """
